...
The companion text is Rudin, Chapter~2, especially \R{2.18}--\R{2.33}.
...
This is the indexed-family notation of \R{2.9}.
\end{Obj}
...
This is the union-of-neighbourhoods viewpoint behind \R{2.19} and \R{2.24}.
\end{Obj}
...
\begin{itemize}
  ...
  \item Every real is a cluster point of $\mathbb Q$ (by density, \xrefL{1.7.2}); $\mathbb Q$ is far
    from closed.
\end{itemize}
Compare Rudin's limit-point and closed-set definitions in \R{2.18} and the nearby limit-point
facts in \R{2.20}.
\end{ObjL}
...
\emph{both} ("clopen"). Compare the complement duality and set algebra in \R{2.23}--\R{2.24}.
\end{ObjL}
...
union of open balls centred at its own points (cf.\ \R{2.19}, \R{2.30}).
\end{ObjL}
...
This combines Rudin's open-cover definition \R{2.31} and compactness definition \R{2.32}.
\end{Obj}
...
\end{itemize}
These are direct open-cover failures for Rudin's compactness definition \R{2.32}; compare the
Heine--Borel theorem \R{2.41}.
\end{ObjL}
