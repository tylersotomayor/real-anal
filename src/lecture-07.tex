...
The companion text is Rudin, Chapter~4, especially \R{4.1}--\R{4.26}.
...
\]
This is the interval case of Rudin's extreme-value theorem (\R{4.16}); the proof is deferred
until compactness of images is available in \xref{7.5.2}.
\end{Obj}
...
$E$, and let $f:E\to Y$. We say that the \emph{limit of $f$ as $x\to p$ is $q$}
(\R{4.1}) if for every
...
\end{Obj}
\begin{Prf}{7.1.6}{Proof by the $\eps$--$\delta$ Definition and Its Contrapositive.}{[A2$+$A11]\quad cf.\ \R{4.2}; use $\delta_n=1/n$ in the reverse direction.}
...
Let $f:E\to Y$ and let $p\in E$. The function $f$ is \emph{continuous at $p$} (\R{4.5}) if
\[
  \lim_{x\to p}f(x)=f(p).
\]
...
value is required to be exactly $f(p)$ (cf.\ \R{4.6} when $p$ is a cluster point).
\end{Obj}
...
small ball, there are no points of $E$ near $p$ except $p$ itself (cf.\ \R{4.5}).
\end{ObjL}
...
continuous (\R{4.7}).
\end{Obj}
\begin{Prf}{7.2.4}{Proof by an $\eps$--$\eta$--$\delta$ Chase.}{[A3]\quad cf.\ \R{4.7}.}
...
the point $x$ and $\eps$.
This is a concrete $\eps$--$\delta$ proof of the reciprocal case covered abstractly by
\R{4.9}.
\end{Obj}
\begin{Prf}{7.3.2}{Direct $\eps$--$\delta$ Proof for the Reciprocal.}{[A3]\quad cf.\ \R{4.9}; the point-dependence is the point of the example.}
...
\xrefL{6.6.11} and Rudin's power-series radius theorem \R{3.39}.
\end{ObjL}
...
Compare Rudin's catalogue of discontinuity examples in \R{4.27}.
\end{ObjL}
...
different sequences $x_n\to0$, the values $f(x_n)$ can approach different limits (cf.\ \R{4.27}).
\end{ObjL}
...
This specializes Rudin's sequential limit criterion to continuity (cf.\ \R{4.2}, \R{4.6}).
\end{Obj}
\begin{Prf}{7.4.1}{Proof by Specializing the Limit Criterion.}{[A2$+$A11]\quad cf.\ \R{4.2}; the reverse direction again uses $\delta_n=1/n$.}
...
\[
  ...
  U\subseteq f^{-1}(V).
\]
This is the pointwise neighbourhood form underlying Rudin's global open-set characterization
(\R{4.8}).
\end{Obj}
...
This is Rudin's open-set characterization of continuity (\R{4.8}).
\end{Obj}
\begin{Prf}{7.4.3}{Proof by Pulling Back Open Balls.}{[Direct]\quad cf.\ \R{4.8}.}
...
This is a second proof of \R{4.7}, using \R{4.8}.
\end{Obj}
\begin{Prf}{7.4.4}{Proof by the Open-Set Characterization.}{[Direct]\quad a second proof of \xref{7.2.4}; cf.\ \R{4.8}.}
...
\end{Obj}
\begin{Prf}{7.5.1}{Proof by Pulling Back an Open Cover.}{[A6]\quad cf.\ \R{4.14}; uses \xref{7.4.3}.}
...
\]
This is Rudin's extreme-value theorem for real-valued continuous functions on compact
domains (\R{4.16}).
\end{Obj}
\begin{Prf}{7.5.2}{Proof by Compactness of the Image.}{[A6$+$A5]\quad cf.\ \R{4.16}; uses \xref{7.5.1} and Heine--Borel.}
...
intervals (cf.\ \R{2.45}, \R{2.47}).
\end{Obj}

\begin{Obj}{Theorem}{7.5.4}{Continuous image of a connected set}
If $X$ is connected and $f:X\to Y$ is continuous, then $f(X)$ is connected (\R{4.22}).
\end{Obj}
\begin{Prf}{7.5.4}{Proof by Pulling Back a Separation.}{[A9]\quad cf.\ \R{4.22}; uses \xref{7.4.3}.}
...
This is the intermediate-value theorem (\R{4.23}).
\end{Obj}
\begin{Prf}{7.5.5}{Proof by Connectedness of the Image.}{[Direct]\quad cf.\ \R{4.23}; compactness gives closed and bounded; connectedness gives an interval.}
...
\emph{homeomorphic}. Rudin uses the compact-domain theorem for continuous bijections in
\R{4.17}.
\end{Obj}
...
continuous, so $f$ is a homeomorphism (\R{4.17}).
\end{Obj}
\begin{Prf}{7.6.2}{Proof by Showing the Inverse Pulls Back Opens to Opens.}{[A6]\quad cf.\ \R{4.17}; compactness makes continuous images of closed sets closed.}
...
have arguments tending to $2\pi$, while $\varphi^{-1}(1,0)=0$ (cf.\ \R{4.21}).
\end{Obj}
...
compact, and compact subsets of a metric space are closed (cf.\ \R{4.17}).
\end{ObjL}
...
trefoil knot, the perimeter of a square, the $2$-torus, $[0,1]$, and the unit disc. Compare the
compact-domain homeomorphism theorem \R{4.17} and circle example \R{4.21}.
\end{ObjL}
...
A function $f:X\to Y$ is \emph{uniformly continuous} on $X$ (\R{4.18}) if for every $\eps>0$ there exists
...
If $X$ is compact and $f:X\to Y$ is continuous, then $f$ is uniformly continuous (\R{4.19}).
\end{Obj}
\begin{Prf}{7.7.2}{Proof by a Finite Half-Radius Cover.}{[A6$+$A4]\quad cf.\ \R{4.19}; the half-radius leaves room for nearby points.}
...
uniformly continuous. Compare Rudin's noncompact-domain counterexamples in \R{4.20}.
\end{Obj}
...
(\xref{7.7.2}). These are \R{4.14}, \R{4.17}, and \R{4.19}.
\end{ObjL}
