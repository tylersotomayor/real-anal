...
$0.999\ldots=1$. The companion text is Rudin, Chapter~1, especially the ordered-field and
least-upper-bound material \R{1.5}--\R{1.20}.
...
Rudin treats induction as prerequisite proof background rather than a numbered analysis result.
\end{Obj}
...
This is the proof principle behind later uses of induction; Rudin assumes it as background.
\end{Obj}
...
This is included as induction practice rather than as a Rudin-numbered item.
\end{ObjL}
...
Writing $+n,-n$ for the classes recovers $\mathbb Z=\{\dots,-2,-1,0,1,2,\dots\}$. Rudin treats
$\mathbb Z$ as background and begins the analysis notation with the rational field in \R{1.4}.
\end{Obj}
...
Rudin records the rational-number notation and convention in \R{1.4}.
\end{Obj}
...
$\dfrac ab$ with $\gcd(a,b)=1$ and the sign carried in the numerator. This is the
equivalence-class bookkeeping behind the notation in \R{1.4}.
\end{ObjL}
...
An \emph{order} on a set $S$ is a relation $<$ such that (cf.\ \R{1.5})
...
Rudin calls this an ordered set in \R{1.6}.
\end{Obj}
...
makes it unfit for analysis. Compare Rudin's ordered-field definition \R{1.17} and the real-field
existence theorem \R{1.19}.
\end{ObjL}
...
\end{Obj}
\begin{Prf}{1.4.1}{Proof by Contradiction of the Irrationality of $\sqrt2$.}{[A9]\quad cf.\ \R{1.1}; uses ``$n^2$ even $\Rightarrow n$ even''.}
...
for $E$ if $x\le\beta$ for every $x\in E$ (\R{1.7}).
\end{Obj}
...
$\beta=\sup E=\operatorname{lub}E$ (\R{1.8}).
\end{Obj}
...
implies $\alpha$ is not a lower bound. We write $\beta=\inf E=\operatorname{glb}E$ (cf.\ \R{1.8}). Since
...
This is Rudin's existence theorem for the real field (\R{1.19}).
\end{Obj}
...
$\mathbb R$---between any two reals lies a rational (\xref{1.7.2}; cf.\ \R{1.19}--\R{1.20}).
\end{ObjL}
...
\end{enumerate}
The cuts, suitably ordered, \emph{are} the real numbers; this is one construction behind
Rudin's real-field theorem \R{1.19}.
\end{Obj}
...
is now a bona fide element (cf.\ \R{1.1}, \R{1.19}).
\end{ObjL}
...
This is the Archimedean part of \R{1.20}.
\end{Obj}
\begin{Prf}{1.7.1}{Proof by Contradiction of the Archimedean Property.}{[A5$+$A9]\quad cf.\ \R{1.20}; via the supremum; uses \xref{1.6.1}.}
...
This is the density part of \R{1.20}.
\end{Obj}
\begin{Prf}{1.7.2}{Proof by the Archimedean Property of the Density of $\mathbb Q$.}{[Direct]\quad cf.\ \R{1.20}; uses \xref{1.7.1}.}
...
(\xref{1.7.4}) is the single expansion this rule never produces. The key order input is
\R{1.20}.
\end{ObjL}
