...
Sets $X$ and $Y$ are \emph{equinumerous}, written $X\sim Y$, if there is a bijection $f:X\to Y$. With
...
a bijection that lists $\mathbb Z$ as $0,-1,1,-2,2,-3,3,\dots$.
\end{ObjL}
...
If $X$ is countable and $A\subseteq X$ is infinite, then $A$ is countable.
\end{Obj}
\begin{Prf}{2.1.9}{Proof by Recursive Selection of Indices that the Subset is Countable.}{[A10]\quad the recursion is an induction, justified at each step by $A$ being infinite.}
...
is irrational with probability $1$.
\end{ObjL}
...
and $d_1(f,g)=0$ forces $f=g$ since $|f-g|$ is continuous.
\end{ObjL}
...
the geometry); (3)~verify $B_{r_p}(p)\subseteq E$. The proof of \xref{2.3.5} is the model.
\end{ObjL}
