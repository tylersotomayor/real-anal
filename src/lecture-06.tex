...
A metric space $(X,d)$ is \emph{complete} if every Cauchy sequence in $X$ converges to a
point of $X$.
\end{Obj}
...
\begin{itemize}[leftmargin=1.3em,topsep=2pt,itemsep=1pt,label=\textbullet]
  ...
  \item $\mathbb R^k$ is complete.
  \item $\mathbb Q$ and $(0,1)$ are \emph{not} complete.
\end{itemize}
\end{ObjL}
...
$\mathbb Q$ this fails: a Cauchy sequence may have no limit (\xref{6.1.2}).
\end{ObjL}
...
\end{Obj}
\begin{Prf}{6.2.2}{Proof by a Supremum Argument of the Monotone Convergence Theorem.}{[A5]\quad the decreasing case follows by applying the increasing case to $\{-s_n\}$.}
...
sequence does not converge, since these differ.
\end{ObjL}
...
with $\pm\infty$ admitted as subsequential limits.)
\end{ObjL}
...
$-\infty$, and $\liminf s_n,\limsup s_n$ may equal $\pm\infty$.
\end{ObjL}
...
$\sqrt[n]{p}=1/\sqrt[n]{\sigma}\to1$ (and $\sqrt[n]{1}=1$). Hence $\sqrt[n]{p}\to1$ for every $p>0$.
\end{ObjL}
...
\end{Obj}
\begin{Prf}{6.6.2}{Proof by Completeness of $\mathbb R$ of the Cauchy Criterion.}{[Direct]\quad the partial sums converge iff they are Cauchy, and $\mathbb R$ is complete.}
...
\end{Obj}
\begin{Prf}{6.6.3}{Proof by the Cauchy Criterion with $m=n$.}{[Direct]}
...
\end{Obj}
\begin{Prf}{6.6.4}{Proof by Dyadic Grouping of the Divergence of the Harmonic Series.}{[Direct]\quad Cauchy condensation; each dyadic block sums to $\tfrac12$.}
...
\end{Obj}
\PrfRef{6.6.5}{Proof of the Convergence of the $p$-Series.}{Stated in lecture as a fact, for use as a comparison series.}
...
are bounded.
\end{Obj}
\begin{Prf}{6.6.6}{Proof by Monotone Convergence for a Series of Nonnegative Terms.}{[A5]\quad the partial sums increase; apply \xref{6.2.2}.}
...
\end{Obj}
\begin{Prf}{6.6.7}{Proof by the Partial-Sum Formula of the Geometric Series.}{[Direct]\quad uses $|x|^{k}\to0$ for $|x|<1$ (\xref{6.5.1}).}
...
\end{enumerate}
\end{Obj}
\begin{Prf}{6.6.8}{Proof by the Cauchy Criterion of the Comparison Test.}{[A4]\quad bound the tail of $\sum a_n$ by that of $\sum c_n$.}
...
\end{itemize}
\end{Obj}
\begin{Prf}{6.6.9}{Proof by Comparison with a Geometric Series of the Root Test.}{[A1]\quad uses the comparison test (\xref{6.6.8}).}
...
\end{Obj}
\begin{Prf}{6.6.10}{Proof by Comparison with a Geometric Series of the Ratio Test.}{[A1]\quad uses the comparison test (\xref{6.6.8}).}
